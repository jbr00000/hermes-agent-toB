...
/**
 * 主区域右下角的浮动小猫：跟随当前标签页的运行神态，可被用户拖拽移动，
 * 拖拽位置写入 localStorage 跨会话保留。仅在 chat/agent 标签页出现。
...
export function PetFloat({ tab }: { tab: { type: TabType; refId: string } | null }): React.ReactElement | null {
  if (!tab || (tab.type !== 'chat' && tab.type !== 'agent')) return null
  return tab.type === 'chat'
    ? <ChatPetFloat sessionId={tab.refId} />
    : <AgentPetFloat taskId={tab.refId} />
}

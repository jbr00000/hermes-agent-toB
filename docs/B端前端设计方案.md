...
> **更新（2026-08）**：上述假设已推翻。Hermes 后端已实现企业统一知识库构建链路（上传落盘 `$HERMES_HOME/knowledge/files/` → MinerU/本地解析 → 分块 → ES + Milvus 入库），前端知识库页已对接真实 `/knowledge` 接口；业务空间/个人库/文档级权限仍为后续范围。详见 [`知识库构建方案.md`](知识库构建方案.md)。下文 §8.3/§8.4 的空间-知识库-权限模型是二期目标形态，一期实现为其简化子集（单企业库、admin 维护、全员只读）。

...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under HERMES_HOME/profiles/ that contain a config.yaml."""
...
def test_dispatch_once_prioritizes_dead_pid_crash_detection(kanban_home, monkeypatch):
    """dispatch_once treats a stale task with a dead PID as a crash."""
    import hermes_cli.kanban_db as _kb
    ...
    with kb.connect() as conn:
        ...
        assert t in res.crashed, "Dead worker should appear in result.crashed"
        assert t not in res.stale

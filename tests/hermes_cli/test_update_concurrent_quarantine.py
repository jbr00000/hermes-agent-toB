...
# ---------------------------------------------------------------------------
# Windows gateway pause/resume hooks are no-ops in the to-B fork
# ---------------------------------------------------------------------------


@patch.object(cli_main, "_is_windows", return_value=True)
def test_windows_gateway_update_hooks_are_noops(_winp):
    assert cli_main._pause_windows_gateways_for_update() is None
    assert cli_main._resume_windows_gateways_after_update({"resume_needed": True}) is None
    assert cli_main._cold_start_windows_gateway_after_update() is None

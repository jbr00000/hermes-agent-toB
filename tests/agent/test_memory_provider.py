...
class TestPluginMemoryDiscovery:
    """Memory providers are discovered from user plugin directories."""

    def test_discover_has_no_bundled_providers_by_default(self):
        """The to-B build does not ship bundled third-party memory providers."""
...
class TestUserInstalledProviderDiscovery:
    ...
    def test_user_plugin_loads_without_bundled_collision(self, tmp_path, monkeypatch):
        """User-installed providers are loadable when no bundled provider shadows them."""
        from plugins.memory import load_memory_provider, discover_memory_providers
        plugin_dir = tmp_path / "plugins" / "user_memory"
        plugin_dir.mkdir(parents=True)
        (plugin_dir / "__init__.py").write_text(
            "from agent.memory_provider import MemoryProvider\n"
            "class Fake(MemoryProvider):\n"
            "    @property\n"
            "    def name(self): return 'user_memory'\n"
            "    def is_available(self): return True\n"
            "    def initialize(self, **kw): pass\n"
            "    def sync_turn(self, *a, **kw): pass\n"
            "    def get_tool_schemas(self): return []\n"
        ...
        p = load_memory_provider("user_memory")
        assert p is not None
        assert p.name == "user_memory"

        providers = discover_memory_providers()
        assert [n for n, _, _ in providers] == ["user_memory"]

...
DEFAULT = "You are on Slack. Do not use markdown."
...
class TestResolvePlatformHint:
    def test_no_overrides_returns_default(self):
        assert _resolve_platform_hint(_agent({}), "slack", DEFAULT) == DEFAULT

    def test_missing_attr_returns_default(self):
        a = types.SimpleNamespace()  # no _platform_hint_overrides at all
        assert _resolve_platform_hint(a, "slack", DEFAULT) == DEFAULT
    ...
    def test_append_dict(self):
        a = _agent({"slack": {"append": EXTRA}})
        out = _resolve_platform_hint(a, "slack", DEFAULT)
        assert out == f"{DEFAULT}\n\n{EXTRA}"
        assert DEFAULT in out and EXTRA in out

    def test_replace_dict(self):
        a = _agent({"slack": {"replace": EXTRA}})
        out = _resolve_platform_hint(a, "slack", DEFAULT)
        assert out == EXTRA
        assert DEFAULT not in out

    def test_replace_wins_over_append_but_both_applied(self):
        a = _agent({"slack": {"replace": "BASE", "append": "TAIL"}})
        out = _resolve_platform_hint(a, "slack", DEFAULT)
    ...
    def test_bare_string_is_append_shorthand(self):
        a = _agent({"slack": EXTRA})
        out = _resolve_platform_hint(a, "slack", DEFAULT)
        assert out == f"{DEFAULT}\n\n{EXTRA}"

    def test_other_platform_unaffected(self):
        """An override for slack must not change telegram's hint."""
        a = _agent({"slack": {"append": EXTRA}})

...
The knowledge stack (`knowledge` profile: elasticsearch + milvus + etcd + minio + hermes-knowledge-worker) lives only on `hermes-data` (no outbound); ES/Milvus each map one 127.0.0.1 loopback port (19200/19530) purely for host-run dev, etcd/minio have none. `minio` is Milvus-internal object storage, NOT document storage (uploads land in `$HERMES_HOME/knowledge/files/`). See `docs/知识库构建方案.md`.
...
server/runtime_config.py provider/model/reasoning for headless path (5 providers, incl. kimi-coding)
...
server/knowledge/       enterprise knowledge-base build pipeline: parser_client.py (MinerU /
                        ...
                        · semantic_chunker.py (optional chunk_mode=semantic — local chonkie
                        port: sentence embeddings + Savitzky-Golay minima; recipe template in
                        split_templates/default_ch.json) · pipeline.py · queue.py · worker.py
                        · sync_service.py (MySQL source of
